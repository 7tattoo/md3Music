...
class MediaStoreService {
  static const _channel =
      MethodChannel('com.apple.android.music/media_store');
  ...
  /// 缓存最近一次查询到的设备概览。
  static Map<String, dynamic>? _cachedDeviceSummary;

  /// 获取设备概览（厂商/型号/Android 版本/SDK 级别），供诊断报告使用。
  ///
  /// 走原生 MethodChannel；非 Android 平台或通道不可用时返回 null。
  static Future<Map<String, dynamic>?> getDeviceSummary() async {
    if (kIsWeb) return null;
    if (_cachedDeviceSummary != null) return _cachedDeviceSummary;
    try {
      final result = await _channel.invokeMethod<Map<dynamic, dynamic>>(
        'getDeviceSummary',
      );
      if (result == null) return null;
      final summary = result.map((k, v) => MapEntry(k.toString(), v));
      _cachedDeviceSummary = summary;
      return summary;
    } catch (e) {
      debugPrint('[MediaStoreService] 获取设备概览失败: $e');
      return null;
    }
  }
