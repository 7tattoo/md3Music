...
class _FlowingBackgroundState extends State<FlowingBackground>
  ...
  Future<void> _extractColors() async {
    ...
    try {
      ...
      if (!mounted || _disposed) return;
      final normalized = palette.colors
          .where((c) => HSLColor.fromColor(c).saturation >= 0.15 &&
              HSLColor.fromColor(c).lightness > 0.1 &&
              HSLColor.fromColor(c).lightness < 0.92)
          .take(3)
          .map((c) => HSLColor.fromColor(c)
              .withSaturation(
                  HSLColor.fromColor(c).saturation.clamp(0.55, 0.9).toDouble())
              .toColor())
          .toList();
      // 缓存成功结果，供播放器往返时复用
      _paletteCache[url] = normalized;
      // 兜底：封面未提取到合格色时保留当前默认色，避免流光背景清空
