...
class _PlayerPlaylistDialogState extends State<PlayerPlaylistDialog> {
  ...
  /// 横屏时播放列表宽度占屏幕右半边（避免过宽遮挡播放器）。
  static const double _landscapeRightHalfRatio = 0.5;
  ...
  Widget build(BuildContext context) {
    ...
    // 用 Stack + Positioned + Material 实现对话框：
    // - 横屏：靠右半边显示（不遮挡左侧全屏播放器）
    // - 竖屏：居中显示 300x500
    // 不直接用 Dialog widget，避免某些场景下 Dismissible 动画
    // 与 Dialog 自带 ModalRoute 动画冲突。
    if (isLandscape) {
      return Stack(
        children: [
          Positioned(
            ...
            top: 0,
            bottom: 0,
            width: size.width * _landscapeRightHalfRatio,
    ...
    // 竖屏：居中显示（用 Stack + Center 实现固定尺寸）
    return Stack(
      children: [
        Center(
          child: _buildDialogContent(width: 300, height: 500),
        ),
      ],
    );
  }
  ...
  Widget _buildDialogContent({
    ...
    return Material(
      // 竖屏 Dialog 已有圆角，横屏直角延伸到屏幕边缘
      borderRadius: width > 300 ? BorderRadius.zero : null,
