...
class _SettingsPageState extends State<SettingsPage>
  ...
  Future<void> _loadSettings() async {
    ...
    final depthCoverStrength = await _settingsRepository.getDepthCoverStrength();
    // 读取 3D 封面深度模型导入状态
    final depthModelImported =
        await DepthCoverService.instance.isModelImported();
    // 读取 3D 封面深度图缓存占用
    final depthCacheBytes =
        await DepthCoverService.instance.cache.cacheSizeBytes();
    // 读取锁屏歌词开关
    ...
    final updateReminderEnabled =
        await _settingsRepository.getUpdateReminderEnabled();
    final pendingUpdateVersion =
        await _settingsRepository.getUpdateLastNotifiedVersion();
    ...
    setState(() {
      ...
      _depthModelImported = depthModelImported;
  ...
  /// 导入 3D 封面深度模型：文件选择器选取 → sha256 校验 → 拷入应用目录。
  Future<void> _importDepthModel() async {
    final messenger = ScaffoldMessenger.of(context);
    final files = await FilePicker.pickFiles(type: FileType.any);
    final path = files.isEmpty ? null : files.single.path;
    if (path == null) return; // 用户取消
    if (!mounted) return;
    final ok = await DepthCoverService.instance.importModel(path);
    if (!mounted) return;
    setState(() => _depthModelImported = ok);
    messenger.showSnackBar(
      SnackBar(
        content: Text(ok ? '深度模型导入成功' : '导入失败：不是有效的深度模型文件'),
      ),
    );
  }
  ...
  Future<void> _clearDepthCache() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('深度图缓存已清理')),
    );
  }
      ...
      children: [
        ...
        if (kDepthCoverAvailable) ...[
          ...
          if (_depthCoverEnabled) ...[
            ...
            // search: 3D 封面 模型;导入 模型;深度模型;onnx
            ListTile(
              title: const Text('导入深度模型'),
              subtitle: Text(
                _depthModelImported ? '已导入（优先使用）' : '未导入（使用内置模型）',
              ),
              onTap: _importDepthModel,
            ),
...
Widget _settingsDescription(
  ...
  final singleLine = singleLineLandscape &&
