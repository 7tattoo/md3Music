...
void main() {
  ...
  group('computeState - 字符错位 delay', () {
    test('wordIndex=2 的字活跃时刻晚于 wordIndex=0 的字', () {
      // duration=1000ms：delay 由 word.startTime 决定（KRC 解析自带错位）。
      //   wordIndex=0：startTime=0 → currentTimeMs=0 时 t=0（激活）
      //   wordIndex=2：startTime=800 → currentTimeMs=0 时 t=-0.8（未激活，idle）
      final word0 = makeWord(startTime: 0, duration: 1000, text: '运');
      final word2 = makeWord(startTime: 800, duration: 1000, text: '运');
    ...
    test('wordIndex=2 在 startTime 时刻激活（验证 delay 计算正确）', () {
      // wordIndex=2：startTime=800, 字内 t=0 应在 currentTimeMs=800
      final word = makeWord(startTime: 800, duration: 1000, text: '运');
      final state = effect.computeState(
        word: word,
        currentTimeMs: 800,
        isLastWord: false,
    ...
    test('startTime 越小 delay 越小（字更早激活）', () {
      // duration=1000ms：
      //   startTime=400：currentTimeMs=200 时 t=-0.2（idle）
      //   startTime=100：currentTimeMs=200 时 t=0.1（激活）
      final wordLate = makeWord(startTime: 400, duration: 1000, text: '运');
      final wordEarly = makeWord(startTime: 100, duration: 1000, text: '运');
      final stateLate = effect.computeState(
        word: wordLate,
        currentTimeMs: 200,
        isLastWord: false,
        wordIndex: 1,
      ...
      final stateEarly = effect.computeState(
        word: wordEarly,
        currentTimeMs: 200,
        isLastWord: false,
        wordIndex: 1,
      ...
      expect(stateLate, equals(EmphasizeState.idle));
      expect(stateEarly, isNot(equals(EmphasizeState.idle)));
    });
  });
