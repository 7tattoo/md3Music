...
void main() {
  ...
  group('KugouProvider 多账号管理', () {
    ...
    setUp(() async {
      ...
      // 多账号凭证存储在 flutter_secure_storage，测试环境需注入内存 mock
    ...
    test('switchAccount 切换到指定账号并更新当前账号', () async {
      ...
      // 凭证必须已切换到目标账号（token 有效性校验依赖网络，测试环境
      // HTTP 一律 400，无法验证；结果成功或过期都说明切换流程已执行）
      expect(provider.apiClient.userid, 'user_a');
      expect(provider.isLoggedIn, isTrue);
      expect(
        result,
        anyOf(SwitchAccountResult.success, SwitchAccountResult.tokenExpired),
      );
    });
  });
}
