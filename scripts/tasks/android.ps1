...
.DESCRIPTION
  ...
  最后执行 flutter 分包打包，产出 4 个包：
    - standard flavor（无 3D 封面，不含 onnxruntime so / 深度模型）：v8a / v7a / x64 三包
    - depth3d flavor（3D 全量：内置 onnxruntime so + Depth Anything V2 模型）：v8a 一包
...
.PARAMETER NoPause
  ...
  .EXAMPLE
  .\scripts\md3.ps1 android                # 智能检测 + 打包（4 包：standard 3 包 + depth3d 1 包）
...
if ($SkipFlutter) {
    Write-Host "`n完成（-SkipFlutter）。jniLibs 已就绪，可用 flutter build apk --release --flavor standard|depth3d --split-per-abi --target-platform android-arm64,android-arm,android-x64 手动打包" -ForegroundColor Green
    if (-not $NoPause) { Wait-Exit }
    exit 0
}

Write-Step 'Flutter 分包打包（standard 三 ABI 无 3D + depth3d v8a 全量）'
...
try {
    ...
    # standard flavor：不含 3D 封面（无 onnxruntime so / 深度模型），出 v8a/v7a/x64 三包
    Invoke-Native { flutter build apk --release --flavor standard --split-per-abi --target-platform android-arm64,android-arm,android-x64 -t $target }
    # depth3d flavor：3D 全量包（内置 onnxruntime so + 深度模型资产），仅 v8a 一个包
    Invoke-Native { flutter build apk --release --flavor depth3d --dart-define=ENABLE_DEPTH_3D=true --split-per-abi --target-platform android-arm64 -t $target }
}
finally { Pop-Location }

# 无需重命名：flutter 产出 app-<abi>-<flavor>-release.apk（实测 abi 在前）。
# 清理：仅保留两个 flavor 的目标产物，删除历史（无 flavor / 含引擎标识）产物，避免混淆
...
Get-ChildItem $outDir -Filter '*-release.apk' -ErrorAction SilentlyContinue |
    Where-Object { $_.Name -notmatch '^app-(arm64-v8a|armeabi-v7a|x86_64)-(standard|depth3d)-release\.apk$' } |
    Remove-Item -Force
